...
# Run the MCP server
run:
	@echo "Running STAC MCP server..."
	@if command -v uv >/dev/null 2>&1; then \
		PYTHONPATH=src uv run python -m chuk_mcp_stac.server; \
	else \
		PYTHONPATH=src python3 -m chuk_mcp_stac.server; \
	fi
...
docker-build:
	...
	docker build -t chuk-mcp-stac:latest .
	@echo "Docker image built: chuk-mcp-stac:latest"

# Run Docker container
docker-run:
	@echo "Running Docker container..."
	docker run --rm -p 8002:8002 chuk-mcp-stac:latest
...
publish-test: build
	...
	echo ""; \
	echo "Install with:"; \
	echo "  pip install --index-url https://test.pypi.org/simple/ chuk-mcp-stac==$$version"
...
publish-manual: build
	...
	echo "Install with: pip install chuk-mcp-stac==$$version"
